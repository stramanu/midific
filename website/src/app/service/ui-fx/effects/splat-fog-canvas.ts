...
export function start(canvas: HTMLCanvasElement): FxListeners {
    'use strict';
    ...
    // multipleSplats(Math.round(Math.random() * 20) + 5);
    update();
    ...
    const hexToRgb = (hex: `#${string}`) => {
        return hex.match(/[A-Za-z0-9]{2}/g)!.map((v: string) => parseInt(v, 16));
    }
    const toGlclampf = (r:number, g:number, b:number) => [r/255, g/255, b/255];
    const hexToGlclampf = (hex: `#${string}`) => {
...
"#A8C9E6",
  ...
  "#F9E79F" 
        ...
        return hexToGlclampf(COLORS[Math.floor(Math.random() * COLORS.length)])
    } 

    let moveToId: any;

    function move(point: {x: number, y: number}) {
        clearTimeout(moveToId);

        pointers[0].down = true;
        pointers[0].color = getRandomColor();
        pointers[0].moved = pointers[0].down;
        pointers[0].dx = (point.x - pointers[0].x) * 10.0;
        pointers[0].dy = (point.y - pointers[0].y) * 10.0;
        pointers[0].x = point.x;
        pointers[0].y = point.y;
